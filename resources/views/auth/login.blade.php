    ...
    <form method="POST" action="{{ route('login') }}">
        ...
        <div class="mb-6">
            <input type="email" name="email" placeholder="Email"
                class="border-[#E9EDF4] w-full rounded-md border bg-[#FCFDFE] py-3 px-5 text-base text-body-color placeholder-[#ACB6BE] outline-none focus:border-blue-400 focus-visible:shadow-none" value="{{ old('email') }}" required autofocus />
        </div>
        <div class="mb-6">
        ...
        <div class="mb-10">
            ...
            class="w-full cursor-pointer rounded-md border bg-blue-800 py-3 px-5 text-base text-white transition hover:bg-opacity-90" />
        </div>
    </form>
    @if (Route::has('password.request'))
    <a class="mb-2 inline-block text-base text-blue-600 hover:text-blue-400 hover:underline" href="{{ route('password.request') }}">
        {{ __('Forgot your password?') }}
    </a>
    @endif
    <p class="text-base text-[#adadad]">
        Not a member yet?
        <a class="text-blue-600 hover:text-blue-400 hover:underline" href="{{ route('register') }}">
            Sign Up
        </a>
    </p>
    <div>
